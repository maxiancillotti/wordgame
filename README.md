...
The examples above target the original hosted challenge server; this repo's own
backend implementation lives under `cmd/wordgame` and `internal/`, and differs in
two ways worth knowing before testing it locally:

- **Entry point**: run it with `go run ./cmd/wordgame` (or `make run` /
  `make docker-up`), not `go run .` — see `Makefile` and `docker-compose.yml`.
  It listens on `http://localhost:1337`, same as the original spec.
- **Required headers**: every request must carry `Authorization: Bearer
  maxkit-stub-service-token`, `X-User-Id: <a UUID identifying the player>`, and
  `X-User-Role: client`. See `docs/adl/0003-transport-layer-design-decisions.md`
  for why. Example against the local server:

  ``` sh
  curl -X POST http://localhost:1337/new \
    -H "Authorization: Bearer maxkit-stub-service-token" \
    -H "X-User-Id: 11111111-1111-1111-1111-111111111111" \
    -H "X-User-Role: client"
  {"id":"...","current":"________","guesses_remaining":6}

  curl -X POST http://localhost:1337/guess \
    -H "Authorization: Bearer maxkit-stub-service-token" \
    -H "X-User-Id: 11111111-1111-1111-1111-111111111111" \
    -H "X-User-Role: client" \
    -d '{"id":"<game id from above>","guess":"A"}'
  ```

Design decisions not specified by the challenge (repeat guesses, guessing on a
completed game, per-user game ownership, word selection/seeding, and the auth
model above) are recorded in `docs/adl/`.

**For a full guide to testing this implementation** — swagger-ui "Try it
out", a `curl` walkthrough including every error path, and the automated
`cmd/_smoketest` tour — see [`docs/testing.md`](docs/testing.md).
